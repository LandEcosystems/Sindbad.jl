...
# few processes it doesn't select) at every process slot, and at each slot ALSO test every one of that
# process's approaches against the same, correctly-built-up upstream `land`.
#
...
function checkComputeChain(::Type{T}, forcing, land, helpers) where {T <: LandEcosystem}
    params = T()
    land = @inferred SM.define(params, forcing, land, helpers)
    land = @inferred SM.precompute(params, forcing, land, helpers)
...
# soilWBase variant) than the one the fixed test reference happens to select for that other
# process. That's a combination-compatibility issue, not a bug in the approach under test, so
# it's tagged `:incompatible` and never hard-fails, even scoped (see `reportOutcome`). Anything
# else (UndefVarError -- e.g. a variable the approach's own code forgot to unpack; MethodError;
# type instability; ...) reflects the approach's own code and stays `:bug`-severity (hard-fails
# when scoped).
function outcomeFromException(e, label)
    if isMissingFieldError(e)
        return (; severity=:incompatible, reason="$label found incompatible upstream data (likely needs a different approach for an upstream process than this test's fixed reference selects, not a bug in this approach): " * sprint(showerror, e))
    end
...
# `:incompatible` results (missing upstream data from this test's fixed reference selection for
# some *other* process, not a bug) are always informational. `:bug` results are informational too
...
function reportOutcome(phase_label, outcomes, n_total, scoped, scoped_problems)
    ...
    if !isempty(incompatible)
        @info "$phase_label: $(length(incompatible))/$n_total approaches skipped -- incompatible upstream data for this test's fixed reference selection of some other process, not a bug (see SindbadTEM/test/README.md)" incompatible
    end
    if !isempty(bugs)
...
# Real SINDBAD runs never interleave define/precompute and compute across processes: ALL N
# processes get define+precompute first (in order), and only then, in a wholly separate pass, do
# ALL N get compute (in order) -- see the module-level comment above. So this stays two separate
# passes over `reference_sequence`, each threading its own reference-driven `land` -- not one
# combined per-process loop -- to keep each phase's "upstream land" faithful to what a real run
# actually builds up by that point.
...
- `functions`: which of `"define"`/`"precompute"`/`"compute"`/`"update"` results to report.
  `"compute"` always exercises that approach's own `define`+`precompute` first internally (see
  `checkComputeChain`) regardless of whether `"define"`/`"precompute"` are also requested --
  `compute` can't be meaningfully checked without them. `update` is excluded by default -- it's a
  separate, optional per-timestep path (only invoked when `inline_update` is set in
  `experiment.json`) that most approaches don't override, so checking it by default mostly just
  measures how many approaches happen to inherit the no-op default against a land state most of
  them were never designed to see.
