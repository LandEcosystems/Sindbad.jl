export cMicrobialEfficiencycSoil_texture

#! format: off
@bounds @describe @units @timescale @with_kw struct cMicrobialEfficiencycSoil_texture{T1,T2} <: cMicrobialEfficiencycSoil
...
function define(params::cMicrobialEfficiencycSoil_texture, forcing, land, helpers)
    @unpack_nt begin
...
function precompute(params::cMicrobialEfficiencycSoil_texture, forcing, land, helpers)
    ## unpack parameters
    @unpack_cMicrobialEfficiencycSoil_texture params
    ...
    for fO ∈ c_flow_order
        ...
        c_flow_ME_f_cSoil = repElem(c_flow_ME_f_cSoil, microbial_efficiency, c_flow_ME_f_cSoil, c_flow_ME_f_cSoil, fO)
    end
...
purpose(::Type{cMicrobialEfficiencycSoil_texture}) = "Texture-dependent microbial carbon-transfer efficiency for every transfer leaving the soil carbon pools."

@doc """

	$(getModelDocString(cMicrobialEfficiencycSoil_texture))
...
of the soil carbon pools, with no distinction between pathways. Selecting `_texture` in all
...
"""
cMicrobialEfficiencycSoil_texture
