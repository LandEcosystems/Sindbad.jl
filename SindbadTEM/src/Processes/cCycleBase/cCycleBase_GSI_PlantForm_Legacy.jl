export cCycleBase_GSI_PlantForm_Legacy

#! format: off
@bounds @describe @units @timescale @with_kw struct cCycleBase_GSI_PlantForm_Legacy{
    T1,  # c_τ_Root_scalar
    T2,  # c_τ_Wood_scalar
    T3,  # c_τ_Leaf_scalar
...
function define(params::cCycleBase_GSI_PlantForm_Legacy, forcing, land, helpers)
    ...
    c_model = cCycleBase_GSI_PlantForm_Legacy()
...
function get_c_τ(τ_pf, ::cCycleBase_GSI_PlantForm_Legacy)
    τ_root = τ_pf[1]
    τ_wood = τ_pf[2]
    τ_leaf = τ_pf[3]
    τ_reserve = τ_pf[4]
...
$(getModelDocString(cCycleBase_GSI_PlantForm_Legacy))

---

# Extended help

Frozen baseline: byte-for-byte the same parameters and logic `cCycleBase_GSI_PlantForm`
...
*Created by*
 - ncarvalhais
"""
cCycleBase_GSI_PlantForm_Legacy
