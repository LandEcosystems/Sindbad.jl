export cCycleBase_GSI_Legacy

#! format: off
@bounds @describe @units @timescale @with_kw struct cCycleBase_GSI_Legacy{T1,T2,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12} <: cCycleBase
...
function define(params::cCycleBase_GSI_Legacy, forcing, land, helpers)
...
$(getModelDocString(cCycleBase_GSI_Legacy))

---

# Extended help

Frozen baseline: byte-for-byte the same parameters and logic `cCycleBase_GSI` had
before its turnover/C:N defaults were centralized into `poolConfigurations/GSI.jl`,
...
*Versions*
 ...
 - 1.2 on 11.09.2026 [skoirala]: split off from cCycleBase_GSI as the frozen
   pre-centralization baseline, for side-by-side comparison

*Created by*
 - ncarvalhais
"""
cCycleBase_GSI_Legacy
