export cFlowStress_GSI_TimeScale

#! format: off
@bounds @describe @units @timescale @with_kw struct cFlowStress_GSI_TimeScale{T1, T2} <: cFlowStress
    f_τ::T1 = 0.03 | (0.01, 0.10) | "contribution factor for current stressor" | "fraction" | "day"
    f_rg_pot_threshold::T2 = 0.01 | (-Inf, Inf) | "threshold for f_rg_pot" | "fraction" | ""
end
#! format: on

function define(params::cFlowStress_GSI_TimeScale, forcing, land, helpers)
    @unpack_cFlowStress_GSI_TimeScale params
    @unpack_nt begin
        soilW ⇐ land.pools
...
function compute(params::cFlowStress_GSI_TimeScale, forcing, land, helpers)
    ## unpack parameters
    @unpack_cFlowStress_GSI_TimeScale params
	...
	if f_rg_pot < f_rg_pot_threshold
        # Hold both memory states through the night.
        eco_stressor = eco_stressor_prev
        # slope_eco_stressor = slope_eco_stressor_prev
        slope_eco_stressor = eco_stressor - eco_stressor_prev
    else
        eco_stressor = c_allocation_f_soilW *
                    c_allocation_f_soilT *
                    c_allocation_f_cloud

        slope_now = eco_stressor - eco_stressor_prev
        slope_eco_stressor =
            (one(f_τ) - f_τ) * slope_eco_stressor_prev +
            f_τ * slope_now
    end
...
purpose(::Type{cFlowStress_GSI_TimeScale}) = "Carbon transfer rates between pools based on the GSI approach, using stressors such as soil moisture, temperature, and light."

@doc """

$(getModelDocString(cFlowStress_GSI_TimeScale))
...
*Versions*
 ...
 - 1.3 on 25.09.2026 [xshan]: added time scale factor to the contribution of current stressor to the shedding rate
