export cFlowReserves_GSI_TimeScale

#! format: off
@bounds @describe @units @timescale @with_kw struct cFlowReserves_GSI_TimeScale{T1,T2,T3} <: cFlowReserves
    slope_leaf_root_to_reserve::T1 = 0.14 | (0.033, 0.33) | "Leaf-Root to Reserve" | "fraction" | ""
    slope_reserve_to_leaf_root::T2 = 0.14 | (0.033, 0.33) | "Reserve to Leaf-Root" | "fraction" | ""
    f_τ::T3 = 0.03 | (0.01, 0.10) | "contribution factor for current stressor" | "fraction" | "day"
end
#! format: on

function compute(params::cFlowReserves_GSI_TimeScale, forcing, land, helpers)
    ## unpack parameters
    @unpack_cFlowReserves_GSI_TimeScale params
...
purpose(::Type{cFlowReserves_GSI_TimeScale}) = "Carbon transfer rates between pools based on the GSI approach, using stressors such as soil moisture, temperature, and light."

@doc """

$(getModelDocString(cFlowReserves_GSI_TimeScale))
