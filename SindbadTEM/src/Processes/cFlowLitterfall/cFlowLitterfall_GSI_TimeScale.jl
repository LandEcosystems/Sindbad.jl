export cFlowLitterfall_GSI_TimeScale

#! format: off
@bounds @describe @units @timescale @with_kw struct cFlowLitterfall_GSI_TimeScale{T1,T2} <: cFlowLitterfall
    k_shedding::T1 = 0.14 | (0.033, 0.33) | "rate of shedding" | "fraction" | ""
    f_τ::T2 = 0.03 | (0.01, 0.10) | "contribution factor for current stressor" | "fraction" | "day"
end
#! format: on

function compute(params::cFlowLitterfall_GSI_TimeScale, forcing, land, helpers)
    ## unpack parameters
    @unpack_cFlowLitterfall_GSI_TimeScale params
...
purpose(::Type{cFlowLitterfall_GSI_TimeScale}) = "."

@doc """

$(getModelDocString(cFlowLitterfall_GSI_TimeScale))
