export vegClass_MODIS_IGBP

struct vegClass_MODIS_IGBP <: vegClass end

vegTypeCatalog(::Type{vegClass_MODIS_IGBP}) = Classification_MODIS_IGBP
vegTypeClassification(::Type{vegClass_MODIS_IGBP}) = Classification_SINDBAD

purpose(::Type{vegClass_MODIS_IGBP}) = "Gets the vegetation type from IGBP-classified forcing data."

@doc """

$(getModelDocString(vegClass_MODIS_IGBP))
...
interpreted against the MODIS IGBP legend (`Classification_MODIS_IGBP`) and resolved
to `Classification_SINDBAD`'s canonical vocabulary before being published as
...
*Versions*
 ...
 - 6.0 on 12.09.2026 [skoirala]: renamed from `vegTypes_MODIS_IGBP` (process
   ...
   `vegClassDynamics`) -- see `vegClass_MODIS_IGBP_PlantForm` for the
   grouped variant
